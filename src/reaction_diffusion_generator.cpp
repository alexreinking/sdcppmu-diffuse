...
// https://github.com/halide/Halide/blob/9755e3da001e6c53898241a691259c2a3039e81c/apps/HelloWasm/reaction_diffusion_generator.cpp
#include <Halide.h>
...
public:
    ...
    void generate()
    {
        ...
        blur_y(x, y, c) =
                (clamped(x, y - 2, c) + clamped(x, y - 1, c) + clamped(x, y, c) +
                 clamped(x, y + 1, c) + clamped(x, y + 2, c));
        ...
        blurry_noise(x, y, c) = 0.25f * (noise(x, y, c) + noise(x + 1, y, c) +
                                         noise(x + 1, y + 1, c) + noise(x, y + 1, c));
        ...
        new_state(clobber.x, clobber.y, c) =
                select(radius < 100.0f, blurry_noise(clobber.x, clobber.y, c),
                       new_state(clobber.x, clobber.y, c));
    }
    ...
    void schedule()
    {
        ...
        noise.compute_root().vectorize(x, natural_vector_size<float>());
